...
async function migrate() {
  ...
  try {
    ...
    for (const s of servers) {
      ...
      await pg.query(
        `INSERT INTO "server"."servers" (
          ...
          is_entitled, entitlement_end_date, message_id,
        ...
        [
          ...
          s.entitlement_end_date ?? s.entitlementEndDate ?? null,
          toId(s.message_id ?? s.messageId),
          toBool(s.isDeleted  ?? s.is_deleted  ?? false),
          s.datetime_deleted ?? s.datetimeDeleted ?? null,
          s.date_created ?? s.dateCreated ?? null,
          s.date_updated ?? s.dateUpdated ?? null,
        ]
      );
      serverCount++;
    ...
    for (const q of questions) {
      ...
      await pg.query(
        `INSERT INTO "question"."questions" (
          ...
          created, server_id, message_id, is_deleted, datetime_deleted
        ...
        [
          ...
          q.datetimeDeleted        ?? q.datetime_deleted    ?? null,
        ]
      );
      questionCount++;
    ...
    // reports, adverts, entitlements, purchasables: skipped — broken systems / no relevant data to migrate.
    ...
    console.log('  - reports / adverts / entitlements / purchasables: skipped (broken systems / re-seed manually)');
